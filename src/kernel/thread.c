...
#ifdef __x86_64__
	kregs->rip 		= init_ip; /* Seta a entrada da função que será uma thread. */
	kregs->rflags 	= 0x200; 	/* Para threads do kernel, nós habilitamos as interrupções. */
#else
	kregs->eip 		= init_ip; /* Seta a entrada da função que será uma thread. */
...
void thread_init()
{
	...
	t = thread_create((uintptr_t)keyboard_bh, +1, 100, "keyboard");
	if(t == NULL){
	...
	t = thread_create((uintptr_t)dead_task_collector, +1, 100, "colletor");
	if(t == NULL){
	...
	t->pagedir = cr3_read();
		...
		/* Cria o dead_task_collector para coletar as tarefas mortas. */
	t = thread_create((uintptr_t)troll_task, +1, 101, "trolltask");
	if(t == NULL){
		printf("Nao foi possivel criar a thread para captura tarefas mortas.\n");
	}
	...
	t->pagedir = cr3_read();
		...
		/* Cria o dead_task_collector para coletar as tarefas mortas. */
	t = thread_create((uintptr_t)troll_task2, +1, 101, "trolltask2");
	if(t == NULL){
		printf("Nao foi possivel criar a thread para captura tarefas mortas.\n");
	}
