...
int open(const char *path, unsigned access)
{
    ...
    task_curr->files = fs_link_node(task_curr->files, fs_new_node);
...
int close(uintptr_t handle){
    ...
    /* Certifica se pode ou não chamar a função write. */
...
// Destroi o nodo
    if (fs_node == task_curr->files)
        task_curr->files = fs_node->fs_next_node;

    task_curr->files = fs_destroy_node(fs_node);
...
int write(uintptr_t handle, void *buf, unsigned len){
	...
	/* Chama a função write do descritor. */
...
int read(uintptr_t handle, void *buf, unsigned len){
	...
	/* Certifica se pode ou não chamar a função read. */
	if(fs_node->read == NULL){
		return -1;
	}

	/* Chama a função read do descritor. */
...
int select(uintptr_t handle, unsigned access, unsigned *timeout){
	...
	/* Chama a função select do descritor. */
...
uint32_t congotoxy(uintptr_t handle, uint32_t pos){
	...
	con = fs_node->inode;
	set_con(con);
	/* Seta a posição do cursor do consoles. */
...
uint32_t congetxy(uintptr_t handle){
	...
	/* Pega a posição atual do cursor do console 'con'. */
	res = getxy();

	return res;
}

/* Espera até o fim da execução de um pid(esse pid deve ser de um filho). */
int  wait4pid(uint32_t pid){
	...
	for (i = 0;i < NUM_TASKS; i++){
		if (tasks[i].pid == pid){
			...
			// então coloca o processo para dormir.
			if (tasks[i].ppid == task_curr->pid){
				task_curr->status		= TS_BLOCKED;
				task_curr->wait4pid	= true;
...
/* Executa uma aplicação identificada pelo "id". */
int execid(uint32_t id, char **argp, char **envp){
	module_t *mod;
	char        *image;
	int 		 r;
	char         *name;
	...
	/* Verifica se o id do módulo é valido. */
	if (id >= multiboot_info->mods_count){
		return -1;
	}
	/* Pega o endereço do módulo. */
	mod	  = (module_t *)(multiboot_info->mods_addr) + id;

	/* Endereço do inicio do módulo. */
	image = (char *)(mod->mod_start);
	name  = (char *)(mod->string);

	/* Cria a tarefa. */
	if((r=task_create(name, argp, envp, image, task_curr->pid)) > 0){
		return r;
	}
...
int exec(char *name, char **argp, char **envp)
{
	char lista[31];
	unsigned i=1, j;
...
int syscall(uregs_t *regs){
	...
	/* Para threads do kernel não é preciso fazer a tradução de endereços. */
	if(task_curr->as != NULL){
		/* Valor do endereço virtual das aplicações (ring3), só aplicado em EBX. */
		uvirt_to_kvirt = (unsigned)task_curr->as->user_mem - task_curr->as->virt_adr;
		/* Se EBX for 0, não é preciso fazer tradução nenhuma. */
		if(regs->ebx == 0){
			uvirt_to_kvirt = 0;
		}
	}
	...
	switch(regs->eax){
		/* Syscalls para manipulação de descritores. */
		case SYSCALL_OPEN:
		...
		/* Syscalls para manipulação de consoles. */
		case SYSCALL_CLEAR:
		...
		/* Syscalls para manipulação de processo. */
		case SYSCALL_EXIT:
			taskexit(regs->ebx);
		break;
		case SYSCALL_GETPID:
			regs->eax = task_curr->pid;
		break;
		case SYSCALL_GETPPID:
			regs->eax = task_curr->ppid;
		break;
		case SYSCALL_WAIT4PID:
			regs->eax = wait4pid(regs->ebx);
		break;
		case SYSCALL_EXECID:
			regs->eax = execid(regs->ebx, NULL, NULL);
		break;
		...
		break;
		case SYSCALL_EXEC:
			regs->eax = exec((char *)(regs->ebx + uvirt_to_kvirt), NULL, NULL);
		break;
		case SYSCALL_EXECP:
		{
			char **argp = NULL;
			...
			regs->eax = exec((char *)(regs->ebx + uvirt_to_kvirt), argp, envp);
		}
		break;
