...
uint8_t	elf32_reloc_apply(void* image, elf_reloc_t* rel, uint32_t symtab_idx, uint32_t t_sect_idx)
{
	...
	if(rel->symtab_index == 0)	// Referência: (If the index is STN_UNDEF, the undefined symbol index, the relocation uses 0 as the "symbol value.")
	{
		sym_val = 0;
		goto	ignore_symbol;
	}
	...
	{
		if(!elf32_get_symbol_i(image, rel->symtab_index, symtab, &symbol))	// carrega o símbolo a ser relocalizado.
...
ignore_symbol:
		rel_val = (uint32_t*)(image + sect->offset + rel->adr);	// Endereço da relocalização
